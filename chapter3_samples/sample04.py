...
def main(page: ft.Page):
        ...
        if e.path:
            ...
            dialog = ft.AlertDialog(
                ...
                actions=[ft.TextButton(
                    "OK", on_click=lambda _: page.close(dialog))],
            )
            page.open(dialog)
        else:
    ...
    save_file_dialog = ft.FilePicker(
        on_result=event_save_file_dialog)
    load_file_dialog = ft.FilePicker(
        on_result=event_load_file_dialog)
    ...
    text_field_save_file_path = ft.TextField(
        label="保存ファイルパス", width=500, read_only=True)
    text_field_load_file_path = ft.TextField(
        label="読込ファイルパス", width=500, read_only=True)

    # ボタンの定義
    # Note: FilePickerでのon_clickのメソッド指定はlambda式を利用
    button_save_dialog = ft.ElevatedButton(
        "保存の実行", on_click=lambda _: save_file_dialog.save_file(
            "ファイル保存", allowed_extensions=["txt"]))
    button_load_dialog = ft.ElevatedButton(
        "読込の実行", on_click=lambda _: load_file_dialog.pick_files(
            "ファイル読込", allow_multiple=False))

    # Rowの定義とPageへの追加
    first_row = ft.Row(controls=[
        text_save_dialog, button_save_dialog,
        text_field_save_file_path])
    second_row = ft.Row(controls=[
        text_load_dialog, button_load_dialog,
        text_field_load_file_path])
    page.add(first_row)
    page.add(second_row)
